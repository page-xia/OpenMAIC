...
function retryableByMessage(value: unknown): boolean {
  ...
  return /rate limit|too many requests|timeout|timed out|fetch failed|network|ECONNRESET|ECONNREFUSED|ECONNABORTED|ETIMEDOUT|ENOTFOUND|EPIPE|socket hang up|overload|temporarily unavailable|service unavailable|bad gateway|gateway time-?out|internal server error|try again/i.test(
    message,
  );
}
